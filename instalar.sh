...
function InstAsk(){
 ...
 echo -e "$bar2"
 read -p "$(echo -e "${blanco}[Contraseña]: ${cierre}")" -e -i dankel DatabasePass
 echo -e ""
 echo -e "$bar1"
 ...
 echo -e "$bar2" 
 read -p "$(echo -e "${blanco}[Nombre]: ${cierre}")" -e -i dankel DatabaseName
echo -e ""
 echo -e "$bar1"  
 echo -e "${azul}Ingrese su nombre de dominio${cierre}"
 echo -e "$bar2"
 read -p "$(echo -e "${blanco}[Dominio]: ${cierre}")" -e -i tunnelmx.xyz ydomain
 echo -e ""
 echo -e "$bar1"  
...
function InstApache(){
 ...
 wget -O /etc/apache2/sites-enabled/dankel.conf "https://www.dropbox.com/s/9vq2gvyh8exbft5/domain_conf.conf"
...
function InstImpSql(){
 # Pull Fixed SQL
 wget -O /home/panel/html/ypanel.sql https://www.dropbox.com/s/m2qypm3mh1r63sl/sql_v2ray.sql
...
function InstPassword(){
  read -p "$(echo -e "${blanco}[Ingresa la Palabra Secreta]: ${cierre}")" -e -i dankel pwdo
  echo -e "$bar1"
if test $pwdo == "dankel"; then
...
function ScriptMessage(){
 ...
 echo -e " [\e[1;32m$MyScriptName INSTALADOR VPS\e[0m]"
  echo -e "$bar2"
 echo -e "${blanco}[${verdeR}PAYPAL${blanco}]${cierre} ${azul}romanherbert180494@gmail.com${cierre}"
echo -e "$bar1"
 echo -e ""
}
#RCONJOB DE PANEL
...
echo -e "${blanco}Password: ${verdeR}dankel123${cierre}" | tee -a log-install.txt
echo -e "$bar1" | tee -a log-install.txt
echo -e "" | tee -a log-install.txt
